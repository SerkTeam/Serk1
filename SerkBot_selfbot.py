...
import time,random,sys,json,codecs,threading,glob,re,ast,os,subprocess,requests,tempfile
...
Creator="ucbd1374cb68242824dc21abcd7d6f669"
admin=["ucbd1374cb68242824dc21abcd7d6f669",mid]
...
#--------------------INVITE_INTO_ROOM--------------------
        ...
        if op.type == 17:
                ...
                cb = Message()
                cb.to = op.param1
                cb.text = "Hallo " + cl.getContact(op.param2).displayName + " [NewMemb]\n\nSelamat Datang" + cl.getContact(op.param2).displayName + " Di [" + group.name + "]\nJGN NAKAL OK!!" + "\n\nCreator Group => " + group.creator.displayName
                cl.sendMessage(cb)
        if op.type == 15:
            if op.param2 in Bots:
                return
            cl.sendText(op.param1,random.choice(KAC).getContact(op.param2).displayName  +  "Good Bye" + "Semoga Tenang Disana")
            print "MemberLeft"
...
#--------------------------------------------------------
                    ...
                    cl.sendText(msg.to,"Jangan Lupa Di Add Ya!!!")
                    cl.sendMessage(msg)
#--------------------------------------------------------
		...
		gCreator = ginfo.creator.mid
                ...
                cl.sendMessage(msg)
		cl.sendText(msg.to,"My Creator Group")
#--------------------------------------------------------
...
#--------------------------------------------------------
		...
		if wait["AutoKick"] == True: md+="◑ Autokick : on\n"
                ...
                cl.sendText(msg.to,"=====[Status Serk Bot]=====\n"+md)
#--------------------------------------------------------
...
#--------------------------------------------------------
		    ...
		    try:
		        print "activity"
		        url = cl.activity(limit=1000)
		        print url
		        cl.like(url['result']['posts'][0]['userInfo']['mid'], url['result']['posts'][0]['postInfo']['postId'], likeType=1001)
		        cl.comment(url['result']['posts'][0]['userInfo']['mid'], url['result']['posts'][0]['postInfo']['postId'], "Auto like by: http://line.me/ti/p/~rd.rmdhn")
